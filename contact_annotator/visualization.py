...
def draw_landmarks(
    img, points, connections=[], color=(0, 255, 0), size=2
):
    points = points[:,:2]
    for point in points:
        x, y = point
...
def draw_normalized_hand_landmarks_on_cropped(
    img,
    landmark,
    ...
    points = landmark[:, :2].numpy().copy()
    points *= [[image_width, image_height]]

    draw_landmarks(
        img, points, connections=hand_connections,
        color=color,
        size=size
    )
