...
def mediapipe_out_to_blazehand_in(
    ...
    bbox_height = int((bbox_b - bbox_t))
    bbox_width  = int((bbox_r - bbox_l))
...
def mediapipe_out_to_blazehand_in2(
    image,
    landmark_arr, # direct from mediapipe
    bbox
) :
    """
    return landmark : normalized.
    ...
    palm_pos = landmark_arr[0][:2]
    fing_pos = landmark_arr[[8, 12, 16, 20], :2].mean(axis = 0)
    theta = np.arctan2(*(palm_pos - fing_pos))
